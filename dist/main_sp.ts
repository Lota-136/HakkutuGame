...
let digTargetX: number = 0;
let digTargetY: number = 0;
let tool: number = 1;
let hp: number = 0;
let item: number;
let isGameClear: boolean;
let isDamaged: boolean = false;  // この番ダメージを受けたか
let isExcavated: boolean = false;    // この番土を掘ったか
...
let particles: ParticlesContext[] = [];

let damageAnimation: number = 0;
let tutorialPage: number = 1;
let tutorialAnimation: number = 0;
let startTime: number;
let timer: number;
let timerMinites: string;
let timerSeconds: string;
let isCollectionDetail: boolean = false;
let ls_Ponyhaniwa: boolean;
let ls_horsehaniwa: boolean;
let ls_kotohaniwa: boolean;
let ls_househaniwa: boolean;
let ls_magatama: boolean;
let ls_seijikouro: boolean;
...
// 道具ボタンの表示更新
function updateToolButtons(activeTool: number): void {
    const buttons = document.querySelectorAll(".tool-btn");
    buttons.forEach(function(btn) {
        btn.classList.remove("active");
    });

    const activeBtn = document.getElementById(`tool-${activeTool}`) as HTMLElement;
    if (activeBtn) {
...
function createButtons(): void {
    ...
    const actionContainer = document.getElementById("action-container") as HTMLElement;
...
function excavate(field: number[][], x: number, y: number) {
    // 盤面の範囲チェック
    if (x < 0 || x >= 9 || y < 0 || y >= 9) {
        return;
    }
...
// 複数行のテキストを描画
function fillTextLine(text: string, x: number, y: number, charNum: number) {
    let textLine = [];
...
function createParticles(x: number, y: number, img: HTMLImageElement) {
    const pieces = [
        { sx: 8, sy: 4, vx: 3, vy: 0 },
        { sx: 0, sy: 4, vx: -3, vy: 0 },
        { sx: 4, sy: 0, vx: 0, vy: -3 },
...
function playSound(snd: HTMLAudioElement) {
    snd.pause();
    snd.currentTime = 0;
    snd.play();
}
...
function doDigAction() {
    ...
    for (let y = 0; y < field.length; y++) {
        for (let x = 0; x < field[y]!.length; x++) {
            if (field[y]![x] != 0) isGameClear = false;
        }
    }
}
...
function saveLocalStorage(key: string, flag: boolean) {
