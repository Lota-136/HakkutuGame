...
const explain = [
    ...
    "子馬のはにわです。他の馬のはにわにある鞍（くら）やたてがみがなく、発見されたときは「子犬形埴輪」　　として紹介されていました。",
    "横から見た動物のように見えることから獣形勾玉と　　呼ばれており、弥生（やよい）時代前期のものと　　　考えられています。",
...
let toolAnimation: number = 0;
let damageAnimation: number = 0;
let tutorialPage: number = 0;
...
let ls_Ponyhaniwa: boolean = false;
let ls_horsehaniwa: boolean = false;
let ls_kotohaniwa: boolean = false;
let ls_househaniwa: boolean = false;
let ls_magatama: boolean = false;
let ls_seijikouro: boolean = false;
...
function excavate(field: number[][], x: number, y: number): void
{
    if (field[y]![x] == 2) {
        field[y]![x] = 1;
...
function fillTextLine(text: string, x: number, y: number, charNum: number): void
{
    let textLine: string[] = [];

    // textをcharNumごとに分け、textLineに追加
...
function createParticles(x: number, y: number, img: HTMLImageElement): void 
{
    const pieces = [
        { sx: 8, sy: 4, vx:  3, vy:  0 },
        { sx: 0, sy: 4, vx: -3, vy:  0 },
...
function playSound(snd: HTMLAudioElement): void 
{
    snd.pause();
    snd.currentTime = 0;
    snd.play();
...
function saveLocalStorage(key: string, flag: boolean): void 
{
    const value = flag.toString();
    localStorage.setItem(key, value);
}

function loadLocalStorage(key: string): boolean 
{
